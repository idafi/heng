...
// this is a clone of SDL_Scancode, so that client systems don't need to depend on SDL
typedef enum
{
	KEY_UNKNOWN = 0,
...
// this is also a clone of SDL_GameControllerButton
typedef enum
{
	CBUTTON_INVALID = -1,
	CBUTTON_A = 0,
...
// this is also a clone of SDL_GameControllerAxis
typedef enum
{
	AXIS_INVALID = -1,
...
#define INPUT_CONTROLLERS_MAX 4
...
typedef struct
{
	...
	{
		int x, y;
		bool buttons[MBUTTON_COUNT];
	} mouse;

	struct controller_state
	{
		bool buttons[CBUTTON_COUNT];
		int16 axes[AXIS_COUNT];
	} controllers[INPUT_CONTROLLERS_MAX];
} input_state;
