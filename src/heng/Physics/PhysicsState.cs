		...
		public PhysicsState(IEnumerable<RigidBody> rigidBodies, IEnumerable<StaticBody> staticBodies)
		{
			if(rigidBodies != null)
			{
				if(staticBodies != null)
				{
					...
					if(newStaticBodies.RemoveAll(b => b == null) > 0)
					{ Log.Error("found and removed null StaticBodies while constructing PhysicsState"); }
					...
					foreach(RigidBody b in rigidBodies)
					{
						...
						{ Log.Error("couldn't add RigidBody to PhysicsState: body is null"); }
					}
				...
				{ Log.Error("couldn't construct PhysicsState: staticBodies collection is null"); }
			}
			else
			{ Log.Error("couldn't construct PhysicsState: rigidBodies collection is null"); }
		}
	};
}